...
# Install Miniconda and create the conda env `interweave` with Python 3.11.13
RUN curl -fsSL https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh -o /tmp/miniconda.sh \
    && bash /tmp/miniconda.sh -b -p ${CONDA_DIR} \
    && rm -f /tmp/miniconda.sh \
    && conda config --system --set always_yes yes --set changeps1 no \
    && conda update -n base -c defaults conda \
    && conda create -n interweave python=3.11.13 \
    && conda clean -afy

# Activate env by default
ENV CONDA_DEFAULT_ENV=interweave
ENV PATH=${CONDA_DIR}/envs/interweave/bin:${PATH}
